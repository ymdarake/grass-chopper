...
└── weeder_ws/src/grass_chopper/            # ROS 2パッケージ
    ...
    ├── setup.py / setup.cfg
    ├── grass_chopper/
    │   ├── obstacle_avoidance.py           # 障害物回避 純粋ロジック (Phase 2)
    │   ├── weeder_node.py                  # 障害物回避 ROS 2 アダプター
    │   ├── coverage_planner.py             # カバレッジ走行計画 (Phase 4b)
    │   ├── coverage_tracker.py             # カバレッジ率追跡 (Phase 4c)
    │   ├── coverage_commander_node.py      # カバレッジ ROS 2 ノード (Phase 4b-e)
    │   ├── coverage_tracker_node.py        # カバレッジ率配信 (Phase 4c)
    │   ├── map_region_detector.py          # 地図自動領域検出 (Phase 4c)
    │   ├── battery_simulator.py            # バッテリー純粋ロジック (Phase 4d)
    ...
    │   ├── test_map_region_detector.py     # 13件
    ...
    │   ├── coverage_params.yaml            # カバレッジ走行パラメータ (Phase 4b)
    ...
    │   └── docking_launch.py               # AprilTag + opennav_docking (Phase 4e)
    ├── urdf/robot_description.urdf.xacro   # ロボットモデル
    ├── models/apriltag_dock/               # AprilTag 36h11 マーカーモデル (Phase 4e)
    └── worlds/
...
```bash
# Mac ホストで純粋ロジックテスト (rclpy 不要, 172テスト)
make test-pure
