...
# マウントされたディレクトリ内に build/install/log を作ると
# パーミッションエラーになるため、VM ローカルに出力する
multipass exec "${VM_NAME}" -- bash -c "\
    mkdir -p ~/weeder_build && \
    source /opt/ros/jazzy/setup.bash && \
    cd ~/weeder_ws && \
    colcon --log-base ~/weeder_build/log \
        build --symlink-install \
        --build-base ~/weeder_build/build \
        --install-base ~/weeder_build/install"
echo "  ビルド完了!"
...
echo "   3. 以下のコマンドを実行:"
echo ""
echo "     source ~/weeder_build/install/setup.bash"
...
echo "   編集後は再ビルドが必要です:"
echo "     multipass exec ${VM_NAME} -- bash -c 'source /opt/ros/jazzy/setup.bash && cd ~/weeder_ws && colcon --log-base ~/weeder_build/log build --symlink-install --build-base ~/weeder_build/build --install-base ~/weeder_build/install'"
echo ""
