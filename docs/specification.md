...
| 種別 | 実行環境 | 件数 | 方法 |
|------|---------|------|------|
| 純粋ロジックテスト | Mac ホスト | 189件 | `make test-pure` (pytest) |
...
| test_incline_monitor.py | incline_monitor.py | 17 |
...
| incline_monitor.py | 傾斜検知ロジック | `InclineLevel`, `quaternion_to_rpy()`, `evaluate_incline()` |
