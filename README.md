...
ROS 2 Jazzy + Gazebo Harmonic で動く草刈りロボットのシミュレーション環境です。
Mac のブラウザだけで完結する noVNC 方式を採用しており、XQuartz は不要です。
...
Gazebo Harmonic では Classic Gazebo (`libgazebo_ros_*.so`) のプラグインは使いません。
Gazebo ネイティブプラグイン (`gz-sim-*-system`) でセンサー・駆動を処理し、
`ros_gz_bridge` で ROS 2 トピックに変換するアーキテクチャです。

## 前提条件

- macOS (Intel / Apple Silicon)
- [Multipass](https://multipass.run/) がインストール済み
...
# 2. VM を作成してセットアップ (初回は15〜25分)
bash run_sim.sh

# 3. 表示された URL をブラウザで開く
#    例: http://192.168.64.10:6080/vnc.html
#    VNC パスワード: ubuntu

# 4. noVNC デスクトップ内でターミナルを開き、シミュレーション起動
...
ホスト側で編集 → VM 内で再ビルドの流れです。
Multipass マウントによりファイルは自動同期されます。
...
# noVNC ターミナル内で再起動 (Ctrl+C → 再実行)
source ~/weeder_ws/install/setup.bash
ros2 launch grass_chopper sim_launch.py
```

> `--symlink-install` を使っているため、Python ファイルの変更はビルドなしで反映されます。
...
初回は ROS 2 + Gazebo のインストールで 15〜25分 かかります。
進捗を確認するには:
...
# QoS の問題がないか確認 (BEST_EFFORT で配信されている)
...
# マウントし直す
multipass umount ros2-vm:/home/ubuntu/weeder_ws
multipass mount ./weeder_ws ros2-vm:/home/ubuntu/weeder_ws
```
