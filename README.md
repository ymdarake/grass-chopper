...
現在は基本の障害物回避まで実装済み。今後のロードマップは [docs/roadmap.md](docs/roadmap.md) を参照。

**できること (現在):** 緑の草原フィールドで、LiDAR 搭載の小型ロボットが
障害物を自動で避けながら前進する様子をブラウザ越しに観察できます。
...
# noVNC ターミナル内でシミュレーション再起動 (Ctrl+C で停止 → 再実行)
source ~/weeder_ws/install/setup.bash
ros2 launch grass_chopper sim_launch.py
```
